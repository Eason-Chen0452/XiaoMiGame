...
public final class MemorySizeCalculator
{
  ...
  MemorySizeCalculator(Context paramContext, ActivityManager paramActivityManager, ScreenDimensions paramScreenDimensions, float paramFloat1, float paramFloat2, int paramInt, float paramFloat3, float paramFloat4)
  {
    ...
    if (j + i <= k)
    {
      ...
      {
        paramContext = new StringBuilder().append("Calculation complete, Calculated memory cache size: ").append(toMb(this.memoryCacheSize)).append(", pool size: ").append(toMb(this.bitmapPoolSize)).append(", byte array size: ").append(toMb(this.arrayPoolSize)).append(", memory class limited? ");
        if (j + i <= paramInt) {
          break label287;
        }
      }
    ...
    {
      Log.d("MemorySizeCalculator", bool + ", max size: " + toMb(paramInt) + ", memoryClass: " + paramActivityManager.getMemoryClass() + ", isLowMemoryDevice: " + isLowMemoryDevice(paramActivityManager));
      return;
...
/* Location:              C:\Users\Administrator\Desktop\apk\dex2jar\classes-dex2jar.jar!\com\bumptech\glide\load\engine\cache\MemorySizeCalculator.class
