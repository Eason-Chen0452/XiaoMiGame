...
public final class ErrorCorrection
{
  ...
  private static String createECCBlock(CharSequence paramCharSequence, int paramInt1, int paramInt2, int paramInt3)
  {
    ...
    {
      k = arrayOfChar[(paramInt3 - 1)] ^ paramCharSequence.charAt(i);
      j = paramInt3 - 1;
      if (j > 0)
      {
        if ((k != 0) && (arrayOfInt[j] != 0)) {
          arrayOfChar[j] = ((char)(arrayOfChar[(j - 1)] ^ ALOG[((LOG[k] + LOG[arrayOfInt[j]]) % 255)]));
        }
        for (;;)
        {
          j -= 1;
      ...
      if ((k != 0) && (arrayOfInt[0] != 0)) {
        arrayOfChar[0] = ((char)ALOG[((LOG[k] + LOG[arrayOfInt[0]]) % 255)]);
      }
      for (;;)
      {
        i += 1;
  ...
  public static String encodeECC200(String paramString, SymbolInfo paramSymbolInfo)
  {
    ...
    for (;;)
    {
      ...
      while (i < m)
      {
        arrayOfInt1[i] = paramSymbolInfo.getDataLengthForInterleavedBlock(i + 1);
        arrayOfInt2[i] = paramSymbolInfo.getErrorLengthForInterleavedBlock(i + 1);
        localObject[i] = 0;
        if (i > 0) {
...
/* Location:              C:\Users\Administrator\Desktop\apk\dex2jar\classes-dex2jar.jar!\com\google\zxing\datamatrix\encoder\ErrorCorrection.class
